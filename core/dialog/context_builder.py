...
class ContextBuilder:
    ...
    def extract_focus_candidates(self, anchor: Dict, focus: Dict) -> List[str]:
        # Анализ может вернуть None/не те типы — нормализуем входы.
        anchor = anchor if isinstance(anchor, dict) else {}
        focus = focus if isinstance(focus, dict) else {}

        anchor_link = anchor.get("anchor_link")
        is_strong_anchor = bool(anchor.get("is_strong_anchor", False))

        focus_points_raw = focus.get("focus_points") or []
        strong_flags_raw = focus.get("is_strong_focus") or []

        if isinstance(focus_points_raw, str):
            focus_points = [focus_points_raw]
        elif isinstance(focus_points_raw, (list, tuple)):
            focus_points = list(focus_points_raw)
        else:
            focus_points = []

        if isinstance(strong_flags_raw, bool):
            strong_flags = [strong_flags_raw] * len(focus_points)
        elif isinstance(strong_flags_raw, (list, tuple)):
            strong_flags = list(strong_flags_raw)
        else:
            strong_flags = []
    ...
    def build(
        ...
        # 0. Timestamp для ломания DeepSeek кеша
        now = datetime.now()
        time_str = now.strftime('%I:%M %p')  # Формат: "02:30 PM"
        p.append(f"Сейчас: {time_str}")
