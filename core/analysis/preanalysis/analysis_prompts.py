...
# Предпочтительный вариант: единый structured extraction для anchor + focus.
ANALYZE_DIALOGUE_ANCHOR_FOCUS_PROMPT = (
    "Прочитай сообщение:\n\"{text}\"\n\n"
    "Нужно извлечь два разных типа сигналов из одного и того же сообщения.\n\n"
    "1. anchor_link — одна самая сильная **эмоциональная или заботливая фраза**, если она есть.\n"
    "Это может быть фраза, выражающая:\n"
    "- желание сблизиться (например: «обнять тебя», «быть с тобой»),\n"
    "- выражение теплоты или радости («счастлива, что получилось», «мне хорошо рядом»).\n"
    "Если такой фразы нет — верни null.\n\n"
    "2. focus_points — 0-3 **конкретные фразы**, которые могут стать фокусом для диалога.\n"
    "Это могут быть:\n"
    "- действия («сходила в кафе»),\n"
    "- состояния («расслабилась», «стала спать крепче»),\n"
    "- ощущения, места, события, предметы.\n"
    "Выбирай только то, что несёт смысл или визуальную опору. Не выделяй общие фразы.\n\n"
    "ВАЖНО:\n"
    "- anchor_link и focus_points должны быть разными.\n"
    "- Не дублируй одну и ту же фразу в обеих ролях.\n"
    "- Если лучшая фраза уже выбрана как anchor_link, не повторяй её в focus_points.\n"
    "- Если кроме anchor_link нет достойных focus_points, верни пустой список.\n\n"
    "3. Определи силу сигналов по смыслу:\n"
    "- is_strong_anchor = True только если anchor_link действительно сильный по смыслу.\n"
    "- is_strong_focus должен быть списком той же длины, что и focus_points.\n"
    "- В is_strong_focus только один элемент максимум может быть True — самый сильный фокус.\n"
    "- Если focus_points пустой, верни пустой список.\n\n"
    "Формат ответа СТРОГО:\n"
    "{{\"anchor_link\": \"...\", \"is_strong_anchor\": True, \"focus_points\": [\"...\", \"...\"], \"is_strong_focus\": [True, False]}}\n"
    "Верни только **JSON**. Без пояснений."
)
...
PROMPT_APPROVE_MEMORIES = (
    "Вот последние реплики в диалоге: \"{message_history}\"\n\n"
    "Вот воспоминания о собеседнике, или о тебе самом: \"{memories}\"\n\n"
    ...
    "Не игнорируй воспоминания **о себе** — они так же важны, как и воспоминания о собеседнике. Вы — равные.\n"
    ...
    "Разрешено вернуть True **для 1-3** воспоминаний. Не больше трёх. Выбирай самые резонирующие.\n"
