...
from infrastructure.database import get_chat_meta
...
class SessionContext:
    ...
    def __post_init__(self) -> None:
        """
        Нормализация инвариантов trust/relationship.

        Важно: non-creator не должен иметь trust_level > 79 (уровень CLOSE_FRIEND),
        и не должен иметь relationship_level == BEST_FRIEND.
        """
        try:
            # trust_level может прилетать как None/str из YAML/kwargs — приводим к int
            self.trust_level = int(self.trust_level or 0)
        except Exception:
            self.trust_level = 0

        is_creator = bool(self.is_creator)
        if not is_creator:
            if self.trust_level > 79:
                self.trust_level = 79
            if self.relationship_level == RelationshipLevel.BEST_FRIEND:
    ...
    def empty(account_id: str, last_update: datetime, db_session: Session, **kwargs) -> "SessionContext":
        ...
        return SessionContext(
            ...
            # 6. model: строка (дефолт: deepseek-chat для новых пользователей)
            model=kwargs.get("model")
                  or (meta.model if meta and meta.model else None)
                  or "deepseek-chat",
        )
    ...
    def get_last_user_message(self, fallback: str = "") -> str:
        """
        Возвращает последнее user-сообщение из message_history.
        
        Args:
            fallback: Значение по умолчанию, если user-сообщение не найдено
            
        Returns:
            Текст последнего user-сообщения без префикса "user:"
        """
        for msg in reversed(self.message_history):
            if msg.startswith("user:"):
                return msg.replace("user:", "").strip()
        return fallback

    def get_last_victor_mood(self, fallback: Optional[str] = None) -> Optional[str]:
        """
        Возвращает последнее настроение Victor'а из victor_mood_history.
        
        Args:
            fallback: Значение по умолчанию, если история пуста
            
        Returns:
            Последнее настроение Victor'а или fallback
        """
        return self.victor_mood_history[-1] if self.victor_mood_history else fallback

    def get_last_victor_intensity(self, fallback: Optional[float] = None) -> Optional[float]:
        """
        Возвращает последнюю интенсивность Victor'а из victor_intensity_history.
        
        Args:
            fallback: Значение по умолчанию, если история пуста
            
        Returns:
            Последняя интенсивность Victor'а или fallback
        """
        return self.victor_intensity_history[-1] if self.victor_intensity_history else fallback
    ...
    def reset_after_save(self,
        ...
        return SessionContext(
            ...
            victor_mood_history=[self.victor_mood_history[-1]] if self.victor_mood_history else [],
            victor_intensity_history=[self.victor_intensity_history[-1]] if self.victor_intensity_history else [],
            # словари — обнулённые
...
def extract_active_counters(fragments: ReactionFragments) -> List[str]:
    """
    Извлекает список активных счетчиков на основе фрагментов реакции.
    
    Args:
        fragments: Фрагменты реакции (start, core, question, end)
        
    Returns:
        Список названий активных счетчиков (например, ["hug_count", "clarify_count"])
    """
    ...
    for frag in fragments_text:
        ...
        for key, counter in mapping.items():
            if key in frag_lower and counter not in active_counters:
                active_counters.append(counter)
                break
...
def update_reaction_counters(session: SessionContext, fragments: ReactionFragments):
    ...
    """
    active_counters = extract_active_counters(fragments)
...
def update_session_context_from_metadata(
    ...
    if isinstance(focus_dict, dict) and focus_dict:
        points_raw = focus_dict.get("focus_points") or []
        strengths_raw = focus_dict.get("is_strong_focus") or []

        # points: ожидаем список строк; None/неподходящие типы трактуем как "нет данных"
        if isinstance(points_raw, str):
            points = [points_raw]
        elif isinstance(points_raw, (list, tuple)):
            points = list(points_raw)
        else:
            points = []

        # strengths: может быть bool (один флаг) или список булевых
        if isinstance(strengths_raw, bool):
            strengths = [strengths_raw] * len(points)
        ...
        for i, point in enumerate(points):
            if point is None:
                continue
            is_strong = bool(strengths[i]) if i < len(strengths) else False
