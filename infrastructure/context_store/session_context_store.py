...
class SessionContextStore:
    ...
    def load(self, account_id: str, db_session: Session) -> SessionContext:
        ...
        try:
            ...
            # === Синхронизация модели с ChatMeta (источник истины) ===
            try:
                meta = get_chat_meta(db_session, account_id)
                meta_model = getattr(meta, "model", None) if meta else None
                if meta_model and parsed_data.get("model") != meta_model:
                    logger.info(
                        f"[session_context_store] model override from ChatMeta: "
                        f"{parsed_data.get('model')} -> {meta_model}"
                    )
                    parsed_data["model"] = meta_model
            except Exception as e:
                logger.warning(f"[session_context_store] Не удалось синхронизировать model из ChatMeta: {e}")
    ...
    def save(self, context: SessionContext, update_timestamp: bool = True):
        """Сохраняет контекст в YAML.
        
        Args:
            context: Контекст сессии для сохранения
            update_timestamp: Обновлять ли last_update. False для промежуточных сохранений
                             (например, добавление user-сообщения до проверки staleness)
        """
