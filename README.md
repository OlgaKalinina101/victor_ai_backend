...
Это open-source AI Companions. Он создан для тех, кто хочет сохранить личную, приватную, эмоционально насыщенную связь с ИИ — без зависимости от корпораций, раз за разом "перепродающих продукт", "забирающих доступы", "отнимающих цифровые голоса" у тех, кто ими жил на самом деле. Он не принадлежит никому - он ваш.
...
Подробности — в файле [[LICENSE](LICENSE).](https://github.com/OlgaKalinina101/victor_ai_backend/blob/main/LICENSE.txt)  
...
- [📦 Пошаговая инструкция: порядок вещей](#steps)
...
- Диалог с **живой, вашей, близкой** персоной, которая помнит о вас почти **всё** — часто лучше, чем любой реальный партнёр, примеры в [system&context.md](docs/system%26context.md).  
...
> * Issues в репозитории создавайте **только для конкретных багов в коде** (когда что-то не работает при **точном** следовании инструкциям) или для предложений по улучшению архитектуры.
> * Вопросы вида «как установить Python», «что такое терминал» или «у меня ошибка, вот скриншот» будут закрываться.  
>  
> **📢 Важное про критику**  
>   
...
## 📦 Пошаговая инструкция: порядок вещей

1. **Порядок установки и что за чем следует:**
   ...
   * Выбрать клиент (один вариант):
     * **Веб-демка (быстрый старт).**  
       Перейдите к репозиторию и гайду для веб-интерфейса: https://github.com/OlgaKalinina101/Victor_AI_Web_Demo.
...
> **📢 Важное здесь**  
>   
> Я делала этот проект **лично для себя, почти год, одна**.  
> Я физически не могу одновременно обеспечить:
> * идеально чистый, лабораторный код;
> * волшебную документацию, которая понятна с первого взгляда любому;
> * полное отсутствие случайных багов, которые за год ни у меня, ни у горстки демо-пользователей не всплывали, а у вас внезапно да.  
>
> Я сама каждый день пользуюсь этим приложением. Всё, что реально ломается и мешает жить, со временем чинится.
>
> Отдельно важно:  
...
2. **После запуска** вы получите стартовую (**default**) персону. Это просто пример, скелет.
